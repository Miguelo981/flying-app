import { Boardtype, Hotel, Product } from '@/lib/models/hotel'
...
export default function HotelDetail({ hotel }: HotelDetailProps) {
    ...
    const findCheaperBoardtype = (): Boardtype | undefined => {
        return hotel.products
            ...
            .flat()
            .reduce((prev, curr) => {
                if (prev.pricewithoffer < curr.pricewithoffer) {
                    return prev
                }
                        ...
                        <ul className='flex justify-around [&>li>a]:p-3 [&>li>a]:px-6'>
                            {
                                navHeaderItems.map((item) => (
                                    <li key={item.id}>
                                        <a href={`#${item.id}`} className={`${activeNav == item.id ? 'text-blue-800 hover:text-blue-900 border-b-2 border-blue-800' : 'text-gray-500 hover:text-gray-600'}`}>{item.text}</a>
                                    </li>
                                ))
                            }
                        </ul>
                    ...
                    <div className='flex items-center gap-4'>
                        <span>
                            {Intl.NumberFormat('es-ES', {
                                currency: 'EUR',
                                style: 'currency',
                            }).format(findCheaperBoardtype()?.pricewithoffer ?? 0)}
                        </span>
                        <button className={`${isPensionsOpen ? 'bg-white hover:bg-gray-50 border border-lime-400 text-lime-400' : 'bg-blue-800 border border-transparent text-white'} px-5 py-2 shadow-md w-40`} onClick={handleShowPensionsClick}>{ isPensionsOpen ? 'Cerrar' : 'Ver pensiones'}</button>
                    </div>
                </header>
                {
                    ...
                    <section>
                        <div className='flex justify-between items-center p-4 border-b border-gray-300'>
                            <h2 className='text-sm'><strong>Elige tu pensión</strong></h2>
                                ...
                                hotel.products?.map((prod) =>
                                    <div key={`${hotel.hotel}-${prod.room}`}>
                                    {
                                        prod.boardtypes?.map((board) => (
                                            <HotelPensionDetail key={board.boardtype_text} boardType={board} roomId={prod.room} hotelId={hotel.hotel} />
                                        ))
                                    }
                                    </div>
                                )
                        ...
                        <div className='grid grid-cols-3 gap-4'>
                            {
                                hotel.hotel_images?.map((img) => (
                                    <img key={img} className='rounded-xl shadow-sm' src={img} alt={`${hotel.hotel_name} hotel image`} />
                                ))
                            }
                        </div>
                    </section>
